...
void main() {
  ...
  setUp(() {
    ...
    dio = TrotxiClientFactory.create(
            baseUrl: 'https://unit.test', tokenStore: store)
        .dio;
  ...
  for (final status in [403, 500, 503]) {
    test('refresh HTTP $status preserves tokens and the server error',
        () async {
      serve((o) => _json(o.path == '/auth/refresh' ? status : 401));
  ...
  for (final failure in ['connection', 'timeout', '500', '401']) {
    test(
        ...
        () async {
      serve((o) {
        if (o.path == '/auth/refresh') return _rotated();
  ...
  for (final path in ['/auth/driver', '/auth/google', '/auth/apple']) {
  ...
  test('malformed successful refresh preserves the existing session', () async {
    serve((o) => _json(o.path == '/auth/refresh' ? 200 : 401));
  ...
  test('direct refresh rejection does not recurse', () async {
    ...
    expect(
        (await _failure(dio
                .post('/auth/refresh', data: {'refreshToken': 'old-refresh'})))
            .error,
        isA<UnauthorizedException>());
    expect(refreshes, 1);
    expect(store.clears, 1);
  ...
  test(
    ...
    serve((o) => _json(401));
    await _failure(
        dio.post('/auth/refresh', data: {'refreshToken': 'a-replaced-token'}));
    expect(refreshes, 1);
    expect(store.clears, 0);
    expect(store.access, 'old-access');
  });
  ...
  for (final status in [200, 401]) {
    test('in-flight refresh $status cannot overwrite or clear a newer login',
        () async {
      serve((o) {
        if (o.path != '/auth/refresh') return _json(401);
        store.access = 'another-login';
        store.refresh = 'another-refresh';
