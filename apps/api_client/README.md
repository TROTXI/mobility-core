...
The implemented replacement surface: every reviewed cutover operation and nothing else. The thirteen deferred operations are deliberately absent. Generated from target-contract.mjs by build-transport-contract.mjs; do not edit this JSON. For the routes enabled on a specific deployment and its server address, read that deployment’s /docs/json.
