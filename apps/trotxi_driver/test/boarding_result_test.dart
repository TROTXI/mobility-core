...
void main() {
  ...
  test('a dead session tells the driver to sign in, not the rider to go away', () {
    final result = const BoardingResult(outcome: BoardingOutcome.sessionExpired);
  ...
  test('only outcomes another attempt could fix are retryable', () {
    ...
    expect(const BoardingResult(outcome: BoardingOutcome.expired).isRetryable, isTrue);
    expect(const BoardingResult(outcome: BoardingOutcome.offline).isRetryable, isTrue);
    expect(const BoardingResult(outcome: BoardingOutcome.failed).isRetryable, isTrue);

    expect(const BoardingResult(outcome: BoardingOutcome.invalid).isRetryable, isFalse);
    expect(const BoardingResult(outcome: BoardingOutcome.reused).isRetryable, isFalse);
    expect(const BoardingResult(outcome: BoardingOutcome.forbidden).isRetryable, isFalse);
  });

  test('already boarded is not an error', () {
    // The rider is aboard, which is the outcome everyone wanted.
    final result = const BoardingResult(outcome: BoardingOutcome.alreadyBoarded);
